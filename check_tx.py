    ...
    for attempt in range(max_retries):
        try:
            ...
            if hasattr(e, 'response'):
                print(f"Response status: {e.response.status_code if e.response else 'No status'}")
...
def update_wallet_history(address, transactions=None, error=""):
    ...
    address_entry = next((item for item in data["addresses"] if item["address"] == address), None)
    if address_entry:
